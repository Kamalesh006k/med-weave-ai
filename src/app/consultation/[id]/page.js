...
export default function ConsultationRoom() {
        ...
        if (recognitionRef.current) {
          ...
          recognitionRef.current.onerror = (event) => {
            ...
            if (event.error === 'network') {
              setToast({ message: "Speech recognition network error. Please check your internet connection.", type: 'error' });
  ...
  useEffect(() => {
    ...
    if (isRecording) {
      interval = setInterval(async () => {
        ...
        // Optimized: Only check if transcript has grown since the last check
        if (textLength > 5 && textLength !== lastProcessedLengthRef.current) {
          try {
            const res = await consultationService.checkRealtime(id, currentText);
            lastProcessedLengthRef.current = textLength;
            if (res?.warning && res.warning !== "SAFE") {
              setRealtimeWarning(res.warning);
            } else if (res?.warning === "SAFE") {
              setRealtimeWarning("SAFE");
            }
      ...
      }, 2500); // Faster interval for near real-time feedback
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
        ...
        {activeTab === 'live' && (
          <div className="w-full flex flex-col p-8 overflow-y-auto max-h-[calc(100vh-140px)]">
            <div className="mb-8">
              ...
              {realtimeWarning && (
                <div className={`w-full mb-6 p-4 rounded-2xl border flex items-center justify-between animate-in slide-in-from-top duration-500 shadow-lg ${realtimeWarning === 'SAFE'
                    ...
                    {diarizedTranscript.split('\n').filter(line => line.trim()).map((line, i) => {
                      ...
                      return (
                        <div key={i} className={`flex flex-col ${isDoctor ? 'items-end' : 'items-start'} animate-in fade-in duration-500`}>
                          ...
                          <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm font-medium shadow-sm border ${isDoctor ? 'bg-indigo-50 border-indigo-100 text-indigo-900 rounded-tr-none' : 'bg-emerald-50 border-emerald-100 text-emerald-900 rounded-tl-none'
                            }`}>
                            {content}
                          </div>
                        </div>
              ...
              {(transcript.trim() || diarizedTranscript.trim()) && !analysis && (
                <div className="mt-12 flex flex-wrap items-center justify-center gap-4 border-t border-slate-100 pt-8">
                  <div className="flex items-center bg-slate-50 p-1.5 rounded-2xl border border-slate-200/60 shadow-sm">
                    <button
                      onClick={toggleRecording}
                      className={`px-6 py-3 rounded-xl flex items-center gap-2.5 transition-all duration-300 ${
                        isRecording
                    ...
                    >
                      ...
                      <span className="font-semibold text-sm tracking-tight">
                        {isRecording ? "Stop Listening" : "Live Clinic"}
                      </span>
                    </button>

                    <button
                      onClick={() => {
                        const scenario = "Patient: Hello doctor, I have been having stomach pain for the past two days. Probably acidity happens all the time. It's a sharp pain on the right side and I feel nauseous too. " +
                                       "Doctor: Yes, yes, acidity only. No need to worry. Are you sure it's getting worse when I move? " +
                                       "Patient: Should I do any scan or blood test? " +
                                       "Doctor: No need, waste of money. I'll just give you some antacids.";
                        setTranscript(prev => prev + scenario);
                        if (!isRecording) setIsRecording(true);
                        setToast({ message: "Simulating appendicitis dismissal scenario...", type: 'info' });
                      }}
                      className="px-5 py-3 rounded-xl flex items-center gap-2 text-slate-600 hover:text-amber-600 hover:bg-amber-50 transition-all duration-200 group"
                      title="Test real-time risk detection"
                    >
                      <Zap className="w-4 h-4 transition-transform group-hover:scale-110" />
                      <span className="font-medium text-sm">System Test</span>
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-8 py-3.5 bg-slate-900 text-white rounded-2xl font-bold text-sm tracking-wide hover:bg-slate-800 transition-all duration-300 shadow-xl shadow-slate-200 flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed group"
                  >
                    {loading ? (
                    ...
                    <span className="font-bold">
                      {loading ? "Constructing Report..." : "Finalize & Report"}
                    </span>
                  </button>
                      ...
                      {!prescriptionResult ? (
                        <div className="space-y-4">
                          ...
                          <button
                            ...
                            className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-2xl text-xs font-bold tracking-tight shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-2 group"
                          >
                            {verifyingPrescription ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4 group-hover:scale-110 transition-transform" />}
                            {verifyingPrescription ? "Auditing Safety..." : "Verify Prescription Safety"}
                          </button>
                        </div>
                      ) : (
                        <div className={`p-5 rounded-2xl animate-in zoom-in-95 duration-200 ${prescriptionResult.status === 'APPROVED' ? 'bg-emerald-50 border border-emerald-100' :
                          prescriptionResult.status === 'CRITICAL' ? 'bg-red-50 border border-red-100' : 'bg-amber-50 border border-amber-100'
                          }`}>
                          <div className="flex items-start gap-4">
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${prescriptionResult.status === 'APPROVED' ? 'bg-emerald-500 text-white' :
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-1">
                                <h5 className={`text-[10px] font-black uppercase tracking-widest ${prescriptionResult.status === 'APPROVED' ? 'text-emerald-700' :
                      ...
                      <div className="pt-6 border-t border-slate-100">
                        ...
                        {!sessionCompleted ? (
                          <button
                            ...
                            className={`w-full py-4 rounded-2xl text-sm font-bold tracking-tight transition-all shadow-xl flex items-center justify-center gap-3 ${prescriptionResult
                              ? 'bg-slate-900 text-white hover:bg-black shadow-slate-200 hover:scale-[1.01]'
      ...
      {/* Toast Notification */}
      {toast.message && (
        <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl shadow-2xl border animate-in slide-in-from-bottom duration-300 flex items-center gap-3 ${toast.type === 'error' ? 'bg-red-50 border-red-100 text-red-600' : 'bg-emerald-50 border-emerald-100 text-emerald-600'
          }`}>
